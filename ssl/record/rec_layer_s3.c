                  ...
                  int create_empty_fragment)
{
    ...
    for (j = 0; j < numpipes; j++) {
        if (BIO_get_offload_tx(s->wbio)) {
            ...
            eivlen = 0;
            if (type != SSL3_RT_APPLICATION_DATA) {
                struct tls_ctrlmsg *ctrl = (void *)outbuf[j];
                ctrl->type = type & 0xff;
    ...
    for (j = 0; j < numpipes; j++) {
        ...
        /* TODO: Create a fake SSL3_RT_HEADER msg to maintain compatibility */
#ifdef SSL_DEBUG
            printf("msg_callback not called for SSL3_RT_HEADER\n");
#endif
        }
        ...
        /* No header when using offload, except type !=
         * SSL3_RT_APPLICATION_DATA
         */
        if (BIO_get_offload_tx(s->wbio)) {
            if (type != SSL3_RT_APPLICATION_DATA) {
                SSL3_RECORD_add_length(&wr[j], sizeof(struct tls_ctrlmsg));
            }
        } else {
    ...
    for (;;) {
        ...
        if (s->wbio != NULL) {
            ...
            if (BIO_get_offload_tx(s->wbio) && type !=
                    SSL3_RT_APPLICATION_DATA) {
                BIO_set_offload_tx_ctrl_msg(s->wbio);
            }
            i = BIO_write(s->wbio, (char *)
                          &(SSL3_BUFFER_get_buf(&wb[currbuf])
                            [SSL3_BUFFER_get_offset(&wb[currbuf])]),
                          (unsigned int)SSL3_BUFFER_get_left(&wb[currbuf]));
            /* To prevent coalescing such as in buffer_write,
             * we flush the BIO
             */
            if (BIO_get_offload_tx(s->wbio) &&
                    type != SSL3_RT_APPLICATION_DATA) {
